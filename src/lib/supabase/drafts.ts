...
function rowToDraft(row: any): InvitationDraft {
  ...
  const theme: InvitationTheme = {
    ...
    fontScale: typeof displayTheme.fontScale === "number" ? displayTheme.fontScale : undefined,
    titleFontScale: typeof displayTheme.titleFontScale === "number" ? displayTheme.titleFontScale : undefined,
    textFontScale: typeof displayTheme.textFontScale === "number" ? displayTheme.textFontScale : undefined,
    coverElement: displayTheme.coverElement,
    coverLogoUrl: displayTheme.coverLogoUrl,
    coverLogoScale: typeof displayTheme.coverLogoScale === "number" ? displayTheme.coverLogoScale : undefined,
    countdownStyle: displayTheme.countdownStyle,
    heroMetaStyle: displayTheme.heroMetaStyle
  };

  const locations: InvitationLocation[] = (row.invitation_locations ?? []).map((location: any) => ({
    id: location.id,
    type: location.type,
    name: location.name ?? "",
...
export async function signInWithEmail(email: string) {
  const supabase = createClient();
  if (!supabase) return { error: { message: "Supabase non e configurato." } };
...
async function uploadImageToLocationBucket(file: File, path: string): Promise<UploadResult> {
  const supabase = createClient();
  if (!supabase) return { status: "error", message: "Supabase non è configurato." };
  ...
  if (!userData.user) return { status: "error", message: "Accedi prima di caricare l'immagine." };
  const { error } = await supabase.storage
    .from("invitation-location-images")
    .upload(path, file, { cacheControl: "3600", upsert: false, contentType: file.type });
  if (error) return { status: "error", message: error.message };
  const { data } = supabase.storage.from("invitation-location-images").getPublicUrl(path);
  return { status: "remote", url: data.publicUrl, message: "Immagine caricata." };
}

export async function uploadLocationImage(draftId: string, locationId: string, file: File): Promise<UploadResult> {
  const supabase = createClient();
  if (!supabase) return { status: "error", message: "Supabase non è configurato." };
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) return { status: "error", message: "Accedi prima di caricare la foto." };
  const extension = file.name.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg";
  return uploadImageToLocationBucket(file, `${userData.user.id}/${draftId}/${locationId}-${Date.now()}.${extension}`);
}

export async function uploadCoverLogo(draftId: string, file: File): Promise<UploadResult> {
  const supabase = createClient();
  if (!supabase) return { status: "error", message: "Supabase non è configurato." };
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) return { status: "error", message: "Accedi prima di caricare il logo." };
  const extension = file.name.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g, "") || "png";
  return uploadImageToLocationBucket(file, `${userData.user.id}/${draftId}/cover-logo-${Date.now()}.${extension}`);
}

export async function uploadInvitationMedia(draftId: string, file: File): Promise<UploadResult> {
  const supabase = createClient();
  if (!supabase) return { status: "error", message: "Supabase non è configurato." };
  ...
  if (!isPhoto && !isVideo) return { status: "error", message: "Carica una foto JPG, PNG o WebP oppure un video compatibile." };
  if (file.size > 50 * 1024 * 1024) return { status: "error", message: "Il file non può superare 50 MB." };
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) return { status: "error", message: "Accedi prima di aggiungere contenuti Social all'invito." };
  ...
  return { status: "remote", url: data.publicUrl, message: "Contenuto Social caricato. Salva la bozza per pubblicarlo." };
}

export async function uploadCustomTemplateImage(file: File): Promise<UploadResult> {
  const supabase = createClient();
  if (!supabase) return { status: "error", message: "Supabase non è configurato." };
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) return { status: "error", message: "Accedi prima di caricare la grafica." };
  const extension = file.name.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg";
  return uploadImageToLocationBucket(file, `${userData.user.id}/custom-templates/template-${Date.now()}.${extension}`);
}

export async function saveDraftToSupabase(draft: InvitationDraft): Promise<SaveResult> {
  const supabase = createClient();
  if (!supabase) {
    return { status: "local", message: "Bozza salvata nel browser. Configura Supabase per il salvataggio account." };
  }

  const { data: userData, error: userError } = await supabase.auth.getUser();
  if (userError || !userData.user) {
    return { status: "local", message: "Bozza salvata nel browser. Accedi per salvarla anche su Supabase." };
  }
  ...
  if (invitationError) return { status: "error", message: invitationError.message };
  ...
  const persistentMedia = draft.media.filter((item) => /^https?:\/\//.test(item.url));
  ...
  if (themeError) return { status: "error", message: themeError.message };
...
export async function loadUserDraftsFromSupabase() {
  const supabase = createClient();
  if (!supabase) return { drafts: [], message: "Supabase non configurato." };
  ...
  const { data, error } = await supabase
    .from("invitations")
    .select("*, invitation_content(*), invitation_sections(*), invitation_locations(*), invitation_media(*), invitation_themes(*)")
    .order("updated_at", { ascending: false });

  if (error) return { drafts: [], message: error.message };
  return { drafts: (data ?? []).map(rowToDraft), message: "" };
}

export async function fetchDraftsFromSupabase() {
...
export async function deleteDraftFromSupabase(draftId: string): Promise<SaveResult> {
  const supabase = createClient();
  if (!supabase) return { status: "local", message: "Invito eliminato dal browser." };

  const { data: userData, error: userError } = await supabase.auth.getUser();
  if (userError) return { status: "error", message: userError.message };
  if (!userData.user) return { status: "local", message: "Invito eliminato dal browser." };
  ...
  if (error) return { status: "error", message: error.message };
  return { status: "remote", message: "Invito eliminato definitivamente." };
}
