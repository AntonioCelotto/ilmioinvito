...
export default function HomePage() {
  return (
    <main className="home-page">
      ...
      <section className="section home-template-section"><div className="section-inner home-section-head"><div><p className="eyebrow">Una grafica per ogni occasione</p><h2>Scegli lo stile del tuo evento.</h2></div><div><p className="muted">Matrimoni, compleanni, battesimi, comunioni, lauree ed eventi privati o aziendali. Ogni template è personalizzabile nei colori e nei caratteri.</p><a href="/templates">Guarda tutti i template →</a></div></div><div className="section-inner home-template-grid">{featuredTemplates.map((template) => <a href="/templates" className="home-template-card" key={template.name}><div className="home-template-phone"><span className="home-template-notch" aria-hidden="true" /><img src={template.image} alt={`Template ${template.name} per ${template.category}`} /></div><div className="home-template-info"><span>{template.category}</span><strong>{template.name}</strong></div></a>)}</div></section>
...
import { HomeInvitePhone } from "@/components/home-invite-phone";
